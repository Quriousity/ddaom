...
[![최신 릴리스](https://img.shields.io/github/v/release/Quriousity/ddaom)](https://github.com/Quriousity/ddaom/releases/latest)

### ⬇ [최신 exe 내려받기](https://github.com/Quriousity/ddaom/releases/latest) — 윈도우 10/11 64비트, 약 167 MB

설치 필요 없습니다. 내려받아 더블클릭하면 됩니다 (첫 화면까지 10~30초 걸립니다).
코드 서명이 없어 **"Windows의 PC 보호"** 파란 창이 뜨는데 `추가 정보` → `실행` 을 누르면 됩니다.
이 경고가 싫거나 Smart App Control 에 막힌다면 아래 **직접 빌드**로 만드세요 —
자기 PC 에서 만든 파일에는 인터넷 표식이 붙지 않아 경고 자체가 뜨지 않습니다.
...
## 직접 빌드하기 (윈도우) — 이대로만 따라 하면 됩니다

위에서 exe 를 받아 쓰면 이 절은 건너뛰어도 됩니다. **직접 만들면 보안 경고가 뜨지 않고,
소스에서 나온 프로그램이라는 것을 스스로 확인할 수 있습니다.**
