...
| 조작 | |
|---|---|
| `R` / `P` / `H` | 사각형 선택 / 폴리곤 선택 / 팬 |
| 드래그 | 사각형 선택 |
| 클릭 여러 번 → 더블클릭(또는 Enter/우클릭) | 폴리곤 선택 |
| `Ctrl+C` | 선택 영역 텍스트 복사 |
| `Ctrl+Shift+C` | 선택 영역 이미지 복사 |
| `Ctrl+Shift+S` | 선택 영역 이미지 저장 |
| `Delete` | 선택 영역을 리댁션 목록에 추가 |
| 우측 패널 "일괄 적용" | 리댁션 실행 → 새 PDF 저장 + 검증 |
| `Ctrl+휠` / `Ctrl+0` / `Ctrl++/-` | 줌 / 맞춤 |
| `PgUp` / `PgDn`, 스페이스+드래그 | 페이지 이동, 팬 |
